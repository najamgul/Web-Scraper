...
def classify_threat(vt_data=None, shodan_data=None, otx_data=None, abuseipdb_data=None, ioc_type=None, user_input=None, google_data=None):
    ...
    try:
        ...
        if has_timeout:
            logger.warning(f" TIMEOUT DETECTED:")
        ...
        if ioc_type == "keyword":
            ...
            if otx_timeout:
                logger.warning(f" OTX timeout for keyword '{user_input}'")
            ...
            if 'error' in otx_data or not otx_data or otx_data.get('source_count', 0) == 0:
                logger.info(f" No OTX data for keyword '{user_input}'")
        ...
        if all(f == 0 for f in features):
            if timeouts_occurred >= 2:
                ...
                logger.warning(f" INSUFFICIENT DATA: {timeouts_occurred} timeouts, returning Unknown")
                return "Unknown"
            ...
            if data_sources_available >= 1:
                ...
                logger.warning(f" NO USEFUL DATA from any source - returning Informational")
                return "Informational"
...
def rule_based_classification(features, ioc_type, data_sources_available=0, timeouts_occurred=0):
    ...
    if timeouts_occurred >= 2:
        logger.info(f" Too many timeouts ({timeouts_occurred}) - returning Unknown")
        return "Unknown"
